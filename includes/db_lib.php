...
R::setup('mysql:host=localhost;dbname=bs3database', 'root', '');
...
class db {
    ...
    function __construct() {
        ...
        if ($this->dbh = mysqli_connect($db_host, $db_user, $db_password, $db_name)) {
            ...
            //mysqli_query($this->dbh, 'SET time_zone = "+0:00"');
            mysqli_query($this->dbh, "SET @@global.time_zone='+00:00'");
        } else {
    ...
    // Perform a generic select and return a pointer to the result
    public function select1r($query) {
//	echo " - in :".$query;
        $result = mysqli_query($this->dbh, $query);
...
//	var_dump($result);
    ...
    // Perform a generic select and return a pointer to the result
    public function select1f($field, $query) {
        $result = mysqli_query($this->dbh, $query . " limit 1");
        $this->error_test("select", $query);
        $row = mysqli_fetch_assoc($result);
        return $row["$field"];
    }
    ...
    public function insertvalues($table, $columns, $values) {
